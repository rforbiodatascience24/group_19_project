...
```{r}
cancer_data <- data_long |> 
  mutate(Cancer = if_else(Dx_Cancer == "Positive",1,0)) |> select(-Time_since_first_diagnosis, -Time_since_last_diagnosis, Dx_CIN) |> drop_na()
...
=======
  theme_half_open(12) + background_grid() + scale_color_discrete(labels = c("0" = "Negative", "1" = "Positive")) + theme(legend.position = "none") +
  labs(color = "Cervical Cancer Present")
...
=======
  ...
  coord_fixed() + # fix aspect ratio to 1:1
  theme_minimal_grid(12) + ggtitle("The data in the PC coordinates")

```
The arrows shows how the features contribute to the principal components analysis. The longer arrows (e.g., Number_of_diagnosis and Number_of_STDs) contribute more to PC1 and PC2, meaning these variables explain a significant amount of variability in the data.
>>>>>>> 96a9036fc9c1cb05ff609e77ef2dd528a459ff8c
...
=======
  theme_minimal_hgrid(12) + ggtitle("The variance explained by each PC")
```
After PC4 and PC5 the variance explained by each PC drops significantly forming an 'elbow'. After PC10 the other components explain very little variance. These components might not be useful for analyzing or modeling.
>>>>>>> 96a9036fc9c1cb05ff609e77ef2dd528a459ff8c
